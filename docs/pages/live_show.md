...
#### Manual Mode
If the leader's page is reloaded, or its connection drops for a moment, it keeps its leadership. The server holds the leader's place for a few seconds, the reconnecting page picks it up again, and followers are not interrupted. This also holds when the leader user has the live view open in more than one browser or on more than one device and all of them reload at once (for example after **Jump To Page** or **Reload Clients**): the one that was leading stays the leader. (Tabs of the same browser share one connection identity, so they count as one.) Leadership is only kept by a page that is still logged in as the leader user, and logging out normally hands it on. (If the login had already expired, it is handed on once that page reloads or closes and its grace period ends.)
