...
class RBACObjectsHandler(BaseAPIController):
    ...
    async def get(self):
        ...
        if not user:
            self.set_status(200)
            await self.finish({
                'objects': [get_registry().get_schema_by_model(o.__class__)().dump(o) for o in objects],
            ...
            with self.make_session() as session:
                ...
                self.set_status(200)
                await self.finish({
                    'objects': [(get_registry().get_schema_by_model(o.__class__)().dump(o),
                                 self.application.rbac.get_roles(user, o).value) for o in objects],
                    'display_fields': self.application.rbac.get_display_fields(mapper)
                })
