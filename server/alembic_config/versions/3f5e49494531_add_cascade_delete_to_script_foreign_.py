...
def downgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX ix_script_line_revision_association_revision_id ON script_line_revision_association_old (revision_id)"
    )
    op.execute(
        "CREATE INDEX ix_script_line_revision_association_line_id ON script_line_revision_association_old (line_id)"
    )

    # Drop current table and rename old one
    op.execute("DROP TABLE script_line_revision_association")
    op.execute(
        "ALTER TABLE script_line_revision_association_old RENAME TO script_line_revision_association"
    )
