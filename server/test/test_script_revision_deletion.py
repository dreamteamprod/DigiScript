...
class TestScriptRevisionDeletion(DigiScriptTestCase):
    ...
    def create_test_data(self):
        ...
        cue_type = CueType(show_id=show.id, prefix="LX", description="Lighting", colour="#FF0000")
    ...
    def test_delete_script_revision_with_associations(self):
        ...
        line_associations = session.query(ScriptLineRevisionAssociation).filter(
            ScriptLineRevisionAssociation.revision_id == self.revision2_id
        ).all()
        self.assertEqual(len(line_associations), 2)
        
        cue_associations = session.query(CueAssociation).filter(
            CueAssociation.revision_id == self.revision2_id
        ).all()
        ...
        if response.code == 200:
            ...
            self.assertEqual(len(line_associations), 0, "Line associations should be deleted")
            
            cue_associations = session.query(CueAssociation).filter(
                CueAssociation.revision_id == self.revision2_id
            ).all()
            self.assertEqual(len(cue_associations), 0, "Cue associations should be deleted")
            ...
            response_body = escape.json_decode(response.body)
            self.fail(f"Script revision deletion failed with status {response.code}: {response_body}")
