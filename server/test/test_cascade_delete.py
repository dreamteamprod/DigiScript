...
from models.script import (
    Script, ScriptRevision, ScriptLine, ScriptLineRevisionAssociation, 
    ScriptLinePart
)
from models.cue import Cue, CueType, CueAssociation
...
class TestCascadeDelete(DigiScriptTestCase):
    ...
    def setUp(self):
        ...
        cursor.execute("DROP INDEX IF EXISTS ix_script_line_revision_association_revision_id")
        cursor.execute("DROP INDEX IF EXISTS ix_script_line_revision_association_line_id")
        cursor.execute("DROP TABLE script_line_revision_association")
        cursor.execute("ALTER TABLE script_line_revision_association_new RENAME TO script_line_revision_association")
        cursor.execute("CREATE INDEX ix_script_line_revision_association_revision_id ON script_line_revision_association (revision_id)")
        cursor.execute("CREATE INDEX ix_script_line_revision_association_line_id ON script_line_revision_association (line_id)")
        ...
        cursor.execute("ALTER TABLE script_cue_association_new RENAME TO script_cue_association")
    ...
    def test_cascade_delete_script_line(self):
        ...
        with db.sessionmaker() as session:
            ...
            revision = ScriptRevision(script_id=script.id, revision=1, description="Test revision")
            session.add(revision)
            session.flush()
            
            # Create script lines
            line1 = ScriptLine(act_id=act.id, scene_id=scene.id, page=1, stage_direction=False)
            line2 = ScriptLine(act_id=act.id, scene_id=scene.id, page=1, stage_direction=False)
            session.add_all([line1, line2])
            session.flush()
            
            # Create script line parts
            part1 = ScriptLinePart(line_id=line1.id, part_index=1, character_id=character.id, line_text="Hello world!")
            part2 = ScriptLinePart(line_id=line1.id, part_index=2, character_id=character.id, line_text="This is a test.")
            session.add_all([part1, part2])
            session.flush()
            
            # Create revision associations
            assoc1 = ScriptLineRevisionAssociation(revision_id=revision.id, line_id=line1.id, next_line_id=line2.id)
            assoc2 = ScriptLineRevisionAssociation(revision_id=revision.id, line_id=line2.id, previous_line_id=line1.id)
            ...
            cue_assoc = CueAssociation(revision_id=revision.id, line_id=line1.id, cue_id=cue.id)
        ...
        with db.sessionmaker() as session:
            parts_count = session.query(ScriptLinePart).filter_by(line_id=line1_id).count()
            rev_assocs_count = session.query(ScriptLineRevisionAssociation).filter_by(line_id=line1_id).count()
            cue_assocs_count = session.query(CueAssociation).filter_by(line_id=line1_id).count()
            
            self.assertEqual(parts_count, 2, "Should have 2 line parts before deletion")
            self.assertEqual(rev_assocs_count, 1, "Should have 1 revision association before deletion")
            self.assertEqual(cue_assocs_count, 1, "Should have 1 cue association before deletion")
        ...
        with db.sessionmaker() as session:
            ...
            next_refs = session.query(ScriptLineRevisionAssociation).filter_by(next_line_id=line1_id).count()
            prev_refs = session.query(ScriptLineRevisionAssociation).filter_by(previous_line_id=line1_id).count()
            ...
            self.assertEqual(rev_assocs_count, 0, "All revision associations should be deleted")
            self.assertEqual(cue_assocs_count, 0, "All cue associations should be deleted")
            self.assertEqual(next_refs, 0, "All next line references should be deleted")
            self.assertEqual(prev_refs, 0, "All previous line references should be deleted")
