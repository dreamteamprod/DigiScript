"""
Utility module for discovering Python modules in both development and PyInstaller environments.
This provides shared functionality for dynamic module discovery.
"""
...
try:
    from utils.pyinstaller_utils import is_frozen, get_resource_path, get_base_path
except ImportError:
    def is_frozen():
...
def discover_modules(package_name, module_name=None):
    """
    Dynamically discover modules in a package, in both development and PyInstaller environments.

    Args:
        package_name (str): The package to search in (e.g., 'controllers', 'models')
        module_name (str, optional): The name of the current module, to avoid self-import

    Returns:
        list: List of module names that were discovered
    """
    get_logger().info(f"Discovering {package_name} modules...")

    if is_frozen():
        # In PyInstaller mode
        get_logger().info(f"Running in PyInstaller mode, using special discovery for {package_name}")
        ...
        try:
            ...
            get_logger().info(f"Found {len(discovered_modules)} {package_name} modules using pkgutil")
        ...
        if not discovered_modules:
            ...
            for name in list(sys.modules.keys()):
                if name.startswith(f"{package_name}.") and name != f"{package_name}.__init__" and name != module_name:
                    discovered_modules.append(name)

            get_logger().info(f"Found {len(discovered_modules)} {package_name} modules in sys.modules")
        ...
        get_logger().debug(f"Found {len(modules)} {package_name} modules in development mode")
        return modules


def has_module_code(module_name):
    """
    Check if a module or package has actual code (not just __init__.py)
    This helps distinguish between empty packages and ones with code.
    """
    try:
...
def import_modules(package_name, module_dict, current_module=None):
    """
    Import all discovered modules from a package and store them in a dictionary.

    Args:
        package_name (str): The package name (e.g., 'controllers', 'models')
        module_dict (dict): Dictionary to store imported modules
        current_module (str, optional): Name of the current module to avoid self-import

    Returns:
        int: Number of successfully imported modules
    """
    # Discover modules
