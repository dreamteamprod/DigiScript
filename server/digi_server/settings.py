    ...
    def validate(self, value):
        ...
        if not isinstance(value, self.val_type):
            if value is None and not self._nullable:
                raise RuntimeError(
                    f"Value for {self.key} cannot be None (is not nullable)"
                )
...
class Settings:
    ...
    def init_settings(self):
        ...
        self.define(
            "db_log_path",
            str,
            os.path.join(self._base_path, "digiscript_db.log"),
            True,
            self._application.regen_logging,
    ...
    def auto_reload_changes(self):
        ...
        for client in self._application.clients.copy():
    ...
    async def set(self, key, item):
        ...
        if changed:
            ...
            await self._application.ws_send_to_all(
                "SETTINGS_CHANGED", "WS_SETTINGS_CHANGED", settings
            )
